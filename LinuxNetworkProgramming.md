...
| D    | 前四位固定为 1110，后面为多播地址 |                | 224.0.0.0 ~ 239.255.255.255  |        |            |                                |
| E    | 前五位固定为 11110, 后面保留      |                |                              |        |            |                                |

在IPv4网络中，**0.0.0.0到223.255.255.255**属于==单播地址==。例如，当一台IP地址为192.168.1.5的主机向IP地址为192.168.1.200的服务器请求网页时，这就是一个单播的例子。

==组播地址==是保留的D类地址，范围从**224.0.0.0到239.255.255.255**。组播常用于需要向多个接收者发送数据的场景，如音视频会议等。并被划分为局部链接多播地址、预留多播地址和管理权限多播地址三类：

-   **局部链接多播地址**范围在 224.0.0.0~224.0.0.255，这是为路由协议和其它用途保留的地址，**路由器并不转发属于此范围的IP包**；
-   **预留多播地址**为 224.0.1.0~238.255.255.255，可用于全球范围（如Internet）或网络协议；
-   **管理权限多播地址**为 239.0.0.0~239.255.255.255，可供组织内部使用，**类似于私有 IP 地址，不能用于 Internet，可限制多播范围**。

==广播地址==是由网络地址和主机部分全为1组成的，而A、B、C类地址的广播地址取决于它们的默认子网掩码**或实际使用的子网掩码**。例如：当子网划分发生时（即使用非默认的子网掩码），广播地址会发生变化。例如，如果C类网络 `192.168.1.0` 被划分为 `/27` 子网，那么新的子网掩码将是 `255.255.255.224`，对应的广播地址将是：

192.168.1.0001 1111 ==  `192.168.1.31`。

255.255.255.255是一个特殊的IP地址，称为**受限的广播地址**。在IPv4网络中，它主要**用于本地局域网内的广播**，不允许路由器转发以该地址为目的地址的数据包，因此其广播作用被限制在了最小的范围内，即本地网络内。当主机发送一个目的地址为255.255.255.255的数据包时，这个数据包会被发送到本地网络上的所有主机，但不会传播到其他的网络段。所以，255.255.255.255主要用于局域网内的广播通信。
...
对于A类地址来说，默认的子网掩码是**255.0.0.0**；对于B类地址来说默认的子网掩码是**255.255.0.0**；对于C类地址来说默认的子网掩码是**255.255.255.0**。D类地址通常用于组播（Multicast），**不指定子网掩码**，因为组播地址本身就已经指明了接收者范围。

==例如：192.168.1.22的ip地址，它的子网掩码是255.255.255.0，那么它的网络地址就是192.168.1，它的主机地址是22。==

-   怎么判断几个ip地址是否属于同一网段？

    将ip地址与子网掩码按位与，如果网络地址相同则属于同一网段。如：

    我们有以下四个IP地址和对应的子网掩码：

    地址1: IP地址 = 192.168.1.10，子网掩码 = 255.255.255.0
    地址2: IP地址 = 192.168.1.20，子网掩码 = 255.255.255.0
    地址3: IP地址 = 192.168.2.10，子网掩码 = 255.255.255.0
    地址4: IP地址 = 192.168.3.10，子网掩码 = 255.255.255.0

    对每个地址的IP地址和子网掩码进行按位与操作，得到网络地址：

    地址1的网络地址 = 11000000.10101000.00000001.00000000 = 192.168.1.0
    地址2的网络地址 = 11000000.10101000.00000001.00000000 = 192.168.1.0
    地址3的网络地址 = 11000000.10101000.00000010.00000000 = 192.168.2.0
    地址4的网络地址 = 11000000.10101000.00000011.00000000 = 192.168.3.0

    地址1和地址2的网络地址相同（192.168.1.0），因此它们属于同一网段。然而，地址3和地址4的网络地址不同（分别是192.168.2.0和192.168.3.0），因此它们不属于同一网段。
