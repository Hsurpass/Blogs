...
FIN_WAIT-2：客户端收到ACK后处于此状态(==半关闭==)，一直等待服务端发送FIN(==此时服务端还可以继续发送数据给客户端==)。

MSL：报文最长生存时间。

TIME_WAIT: 时间等待定时器。如果对方直接发送了`FIN+ACK`则直接进入`TIME_WAIT` 状态，无需经过`FIN_WAIT-2`状态，

==如果服务端主动关闭，则服务端就会有一段时间处于`TIME_WAIT`状态，导致端口被占用不能启动服务。==
...
1. **确保服务端发送完FIN后能够接收到客户端发送的ACK确认报文**

   客户端发送的ACK报文有可能丢失，服务端接收不到确认报文就会重发FIN，直到收到ACK。

   如果不等待2MSL，客户端在发送完ACK后马上退出，一旦服务端没有收到ACK，它就会一直处于==last_ack==的状态，导致连接不会释放。(半关闭)

2. **保证“已失效的报文段”全都失效。**

   因为报文最大的生存时间是MSL，2MSL足够让所有没处理的报文失效。
