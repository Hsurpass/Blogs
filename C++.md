...
1. 修饰**全局变量**：作用域和生命周期都是全局的，不能跨文件使用。
2. 修饰**局部变量**：作用域在函数内，生命周期是全局的，存储在全局数据区。
3. 修饰**普通函数**：不能跨文件使用，只能在当前文件使用，限制了函数的作用域。
4. 修饰**类成员变量**：类内定义，类外初始化，全部对象共享一个static成员变量。静态成员变量是命名空间属于类的全局变量，存储在data区的rw段。
...
因为在调用构造函数时，虚表指针并没有在对象的内存空间中，必须要**构造函数调用完成后才会形成虚表**指针。
